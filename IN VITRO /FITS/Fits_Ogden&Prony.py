...
Nécessite les fichiers d'essais bruts '.tracking.csv' issus de la machine INSTRON.
"""
...
materials = [
    ...
    {   'name': 'Ecoflex 00-50 (26.03 mm)',
        ...
        'cyc_tstart': 275.0, 'cyc_tpeak': 288.35},
...
for mat in materials:
    ...
    # A. Fit Néo-Hookéen (1 paramètre : alpha1 fixé à 2.0)
    def obj_nh(params):
        mu1 = params[0]
    ...
    # B. Fit Ogden (2 paramètres : mu1 et alpha1 libres)
    ...
    bounds_prony_n1 = [(0.0001, 0.999), (1.0, 1000000.0)]
    ...
    bounds_prony_n2 = [(0.0001, 0.999), (1.0, 20000.0), (0.0001, 0.999), (10000.0, 1000000.0)]
    ...
    print("--- NEO-HOOKÉEN (1 paramètre) ---")
    print(f"mu_1    = {mu1_nh:.5f} MPa")
    print(f"alpha_1 = 2.000 (fixé)")
    print(f"R²      = {r2_nh:.4f}")

    print("\n--- OGDEN (2 paramètres) ---")
    print(f"mu_1    = {mu1_ogden:.5f} MPa")
    print(f"alpha_1 = {alpha1_ogden:.3f}")
    print(f"R²      = {r2_ogden:.4f}")
    
    print(f"\n Comparaison R² (Ogden vs Néo-Hookéen) : delta = {r2_ogden - r2_nh:+.5f}")

    print("\n--- PRONY n=1 ---")
    print(f"g_1 = {g1_n1:.4f}, tau_1 = {tau1_n1_ms:.4f} ms")
    print(f"g_inf = {g_inf_n1:.4f}")
    print(f"R²  = {r2_prony_n1:.4f}")

    print("\n--- PRONY n=2 ---")
    print(f"g_1 = {g1_n2:.4f}, tau_1 = {tau1_n2_ms:.4f} ms")
    print(f"g_2 = {g2_n2:.4f}, tau_2 = {tau2_n2_ms:.4f} ms")
    print(f"g_inf = {g_inf_n2:.4f}")
    print(f"R²  = {r2_prony_n2:.4f}")

    print(f"\n Comparaison R² (n=2 vs n=1) : delta = {r2_prony_n2 - r2_prony_n1:+.5f}")
    ...
    axes[0].plot(eps_c[::100], P_pred_nh[::100], 'x', color='k', markersize=5, label='Neo-Hookean')
    axes[0].plot(eps_c[::100], P_pred_ogden[::100], 'o', markerfacecolor='none', color='k', markersize=6, label='Ogden')
