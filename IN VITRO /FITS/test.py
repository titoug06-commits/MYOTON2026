...
pour silicones utilisés dans la simulation LS-DYNA de l'impact MyotonPRO.

Compare les modèles Néo-Hookéen (alpha fixé) vs Ogden (alpha libre) 
ainsi que les séries de Prony (n=1 vs n=2).

Sortie :
  - figures/ : graphiques d'ajustement pour chaque matériau
  - results_parameters.csv : tableau synthétique des paramètres identifiés
"""
...
# ===== Configuration =====
DATA_DIR = './Donnees_Brutes'  # Chemin vers les fichiers .tracking.csv bruts
FIGURES_DIR = './Figures'  # Dossier de sortie des graphiques
RESULTS_CSV = 'results_parameters.csv'  # Fichier de résultats structuré
...
materials = [
    {   'name': 'Ecoflex 00-10 (3.36 mm)',
        'L0': 3.36,
        'cyc_file': os.path.join(DATA_DIR, 'Ecoflex00-10_336_1.Boucle 1.tracking.csv'),
        'rel_file': os.path.join(DATA_DIR, 'Ecoflex00-10_336_relax_test2.steps.tracking.csv'),
        'rel_tstart': 1.0, 'rel_tpeak': 5.25, 'rel_tend': 305.85,
        'cyc_tstart': 640.0, 'cyc_tpeak': 709.0},

    {   'name': 'Ecoflex 00-10 (11.41 mm)',
        'L0': 11.41,
        'cyc_file': os.path.join(DATA_DIR, 'Ecoflex00-10_1141_4.Boucle 1.tracking.csv'),
        'rel_file': os.path.join(DATA_DIR, 'Ecoflex00-10_1141_relax_test1.steps.tracking.csv'),
        'rel_tstart': 0.6, 'rel_tpeak': 4.05, 'rel_tend': 304.60,
        'cyc_tstart': 475.0, 'cyc_tpeak': 519.15},

    {   'name': 'Ecoflex 00-50 (26.03 mm)',
        'L0': 26.03,
        'cyc_file': os.path.join(DATA_DIR, 'Ecoflex00-50_2603_1.Boucle 1.tracking.csv'),
        'rel_file': os.path.join(DATA_DIR, 'Ecoflex00-50_2603_relax_test1.steps.tracking.csv'),
        'rel_tstart': 0.15, 'rel_tpeak': 1.6, 'rel_tend': 302.10,
        'cyc_tstart': 280.0, 'cyc_tpeak': 288.35},

    {   'name': 'Dragon Skin 30 (1.56 mm)',
        'L0': 1.56,
        'cyc_file': os.path.join(DATA_DIR, 'Dragon30_156_1.Boucle 1.tracking.csv'),
        'rel_file': os.path.join(DATA_DIR, 'Dragon30_156_relax_test3.steps.tracking.csv'),
        'rel_tstart': 4.0, 'rel_tpeak': 12.15, 'rel_tend': 313.90,
        'cyc_tstart': 906.0, 'cyc_tpeak': 1003.35},

    {   'name': 'Dragon Skin 30 (2.70 mm)',
        'L0': 2.70,
        'cyc_file': os.path.join(DATA_DIR, 'Dragon30_270_2.Boucle 1.tracking.csv'),
        'rel_file': os.path.join(DATA_DIR, 'Dragon30_270_relax_test1.steps.tracking.csv'),
...
def force_boltzmann_complete(t_array, F_max, t_ramp, g1, tau1, g2, tau2):
    """Modèle Prony n=2 (intégrale de Boltzmann)."""
    g_inf = 1.0 - g1 - g2
...
results = []  # Stockage des paramètres pour export CSV
...
for mat in materials:
    ...
    res_ogden = minimize(obj_ogden, [0.01, 2.0], method='Nelder-Mead',
                         bounds=[(1e-5, 50.0), (0.1, 5.0)],
    ...
    res_prony_n1 = minimize(obj_prony_n1, [0.15, 2000.0], method='Nelder-Mead',
                            bounds=[(0.0001, 0.4), (1.0, 1000000.0)],
    ...
    res_prony_n2 = minimize(obj_prony_n2, [0.15, 2000.0, 0.15, 50000.0], method='Nelder-Mead',
                            bounds=[(0.0001, 0.4), (1.0, 20000.0), (0.0001, 0.4), (10000.0, 1000000.0)],
                            options={'xatol': 1e-6, 'fatol': 1e-12, 'maxiter': 20000, 'maxfev': 20000})
    ...
    print(f"Hyperélastique  | µ₁ = {mu1_nh:.5f} MPa (Neo-Hookean), R² = {r2_nh:.4f}")
    print(f"Viscoélastique  | g₁ = {g1_n1:.4f}, τ₁ = {tau1_n1_ms:.1f} ms (Prony n=1), R² = {r2_prony_n1:.4f}")
    ...
    results.append({
        ...
        'mu1_MPa': mu1_nh,
        'alpha1': 2.0,
        'R2_hyperelastic': r2_nh,
        'g1_n1': g1_n1,
        'tau1_ms_n1': tau1_n1_ms,
        'R2_prony_n1': r2_prony_n1,
        'g1_n2': g1_n2,
        'tau1_ms_n2': tau1_n2_ms,
        'g2_n2': g2_n2,
        'tau2_ms_n2': tau2_n2_ms,
        'R2_prony_n2': r2_prony_n2,
        'prony_model': 'n=2' if r2_prony_n2 - r2_prony_n1 > 0.003 else 'n=1'
    })
    ...
    plt.rcParams.update({
        ...
        "legend.fontsize": 10, "xtick.direction": "in", "ytick.direction": "in",
        "xtick.top": True, "ytick.right": True, "axes.grid": False, "lines.linewidth": 1.5
    })

    fig, axes = plt.subplots(1, 2, figsize=(10, 4.5))
    step_c = max(1, len(eps_c) // 150)
    step = 10 if mat['L0'] == 26.03 else 80
    ...
    text_ogden = f"Neo-Hookean: $\\mu_1={mu1_nh:.4f}$ MPa, $R^2={r2_nh:.4f}$\n\nOgden: $\\mu_1={mu1_ogden:.4f}$ MPa, $\\alpha_1={alpha1_ogden:.2f}$, $R^2={r2_ogden:.4f}$"
    ...
    F_prony_pts_n1 = np.concatenate([(force_boltzmann_n1(t_all_ms, F_max_exp, t_ramp_ms, g1_n1, tau1_n1_ms) / F_max_exp)[mask_charge][::step_charge],
                                      (force_boltzmann_n1(t_all_ms, F_max_exp, t_ramp_ms, g1_n1, tau1_n1_ms) / F_max_exp)[mask_relax][::step_relax]])
    F_prony_pts_n2 = np.concatenate([(F_pred_n2 / F_max_exp)[mask_charge][::step_charge],
                                      (F_pred_n2 / F_max_exp)[mask_relax][::step_relax]])

    axes[1].plot(t_all_ms / 1000, F_all / F_max_exp, 'k-', linewidth=1, alpha=0.9, label='Experimental')          
    axes[1].plot(t_prony_pts / 1000, F_prony_pts_n1, 'x', color='k', markersize=5, label='Prony (n=1)')
    axes[1].plot(t_prony_pts / 1000, F_prony_pts_n2, 'o', markerfacecolor='none', color='k', markersize=6, label='Prony (n=2)')
...
results_df.to_csv(RESULTS_CSV, index=False, sep=';', decimal=',')
